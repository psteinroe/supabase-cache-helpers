...
<Tabs items={["SWR", "React Query"]}>
  <Tab>```bash pnpm add @supabase-cache-helpers/storage-swr ```</Tab>
  <Tab>```bash # Storage is not yet supported for React Query :( ```</Tab>
</Tabs>

If your package manager does not install peer dependencies automatically, you will need to install them, too.

<Tabs items={["SWR", "React Query"]}>
  <Tab>```bash pnpm add swr react @supabase/storage-js ```</Tab>
  <Tab>```bash # Storage is not yet supported for React Query :( ```</Tab>
</Tabs>
