...
```ts
client.from('contact').select('id,name').eq('has_golden_ticket', true).order('name', { ascending: true }).range(0, 10)
```
...
Note that the cache would not be updated if the returned item does not contain `has_golden_ticket`, because the query requires this field to be `true`. To not force you to manually ensure that the mutation retuns all columns queries from that table across your app, the cache helpers *defaults* to querying all columns that are currently present in the cache. For example, if you mutate with
```tsx
