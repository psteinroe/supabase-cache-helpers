...
>(
    ...
    queries: [
      // Count query
      {
        queryKey: baseQueryKey
          ...
          : ['postgrest', 'disabled', 'count'],
        queryFn: async (): Promise<number | null> => {
          if (!queryFactory) return null;
          return fetchCount(queryFactory(), { countType });
        },
        enabled: enabled && !!queryFactory,
        staleTime,
        gcTime,
      },
      // Data query
      {
        queryKey: baseQueryKey
          ...
          : ['postgrest', 'disabled', 'page'],
        queryFn: async (): Promise<Result[]> => {
          if (!queryFactory) return [];
          return fetchPaginatedData(queryFactory(), {
            pageSize,
            offset,
            rpcArgs,
          });
  ...
  const [countResult, dataResult] = results;
  ...
  const isFetching = countResult.isFetching || dataResult.isFetching;
  ...
  return {
    ...
    totalPages,
    setPage,
    nextPage: hasNextPage && !isFetching ? nextPageFn : null,
    previousPage: hasPreviousPage && !isFetching ? previousPageFn : null,
    hasNextPage,
    hasPreviousPage,
    isLoading,
    isFetching,
    error:
      (countResult.error as PostgrestError | null) ??
      (dataResult.error as PostgrestError | null),
