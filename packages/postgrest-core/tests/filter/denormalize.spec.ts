...
describe('denormalize', () => {
  it('should work with wildcard', () => {
    ...
    expect(
      denormalize(paths, {
        ...
        'note_id.relation_id.some': '678',
        'note_id.relation_id.other': '910',
        'note_id.relation_id.columns': '111',
      }),
    ).toEqual({
      note_id: {
        test: '123',
  ...
  it('should work with wildcard on first level', () => {
    const paths = parseSelectParam(
      'some,*,note_id(test,relation_id,rel:relation_id(test))',
    );
  ...
  it('should work with json array of objects and wildcard', () => {
    expect(
      denormalize(
        [
          {
